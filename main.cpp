...
void NullAllocatorTest() {
    ...
    assert(!block);
    assert(block._ptr == nullptr);
    assert(block._size == 0);
}
...
void StackAllocatorTest() {
    ...
    assert(block1._ptr != nullptr);
    assert(block2._ptr != nullptr);
    assert(block1._size == 128);
    assert(block2._size == 256);
    assert(block1 != block2);
    assert(allocator.owns(block2));
    allocator.deallocate(block2);
    assert(block2._size == 0);
    assert(allocator.owns(block1));
    allocator.deallocate(block1);
    Block block3 = allocator.allocate(1024);
    assert(block3._size == 1024);
...
void FallBackAllocatorTest() {
    ...
    assert(block1._size == 250);
    assert(block2._size == 250);
    assert(allocator.owns(block1));
    assert(allocator.owns(block2));
    allocator.deallocate(block2);
    allocator.deallocate(block1);
